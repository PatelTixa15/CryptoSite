...
def signUpPage(request):
    if request.method == "GET":
        ...
        if request.POST['password1'] == request.POST['password2'] and len(request.POST['username'])>0 and len(request.POST['password1'])>5 and len(request.POST['first_name'])>0 and len(request.POST['last_name'])>0 :
            ...
            try:
                ...
                return render(request, 'CryptoSite/signUp.html', {'error': 'Username is already been taken, Please choose another username'})
        else:
            if request.POST['username']=='' or request.POST['password1']=='' or request.POST['first_name']=='' or request.POST['last_name']=='':
...
def confirm(request,rank,coins):
    ...
    return render(request,'CryptoSite/confirm.html',{'user':request.user,'data':data,'image':url,'coins':coins})
...
def buyPage(request,curr_rank):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'CryptoSite/buy.html',{'user':request.user,'data':data,'image':url,'conversion':conversion})
...
def detailPage(request,rank):
    ...
    return render(request,'CryptoSite/detail.html',{'data1':data1,'plot_div':plot_div,'image':url,'active':request.user.is_active})
